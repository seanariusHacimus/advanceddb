...
import { Button, ButtonPrimary, Flex, TitleH1, InputWrapper, Input } from '../../../styles';
import { ReactComponent as IconCheck } from '../../../assets/list-icon.svg';
import { fetchActionPlans } from '../../../store/Actions/actions';
import moment from 'moment-timezone';
import { ErrorAlerts, InputErrors, parseErrors, indexBy } from "../../../utils";
import { SubActionBase } from "./AddSubAction";
import { errorsConfig, getAllTags, getAllGroupAccounts, initialState, } from "./AddActionPlan";
...
class EditActionPlan extends SubActionBase {
  ...
  async componentDidMount() {
    const { name, start_at, end_at, responsive_accounts, responsive_tags, id, attachments, } = this.props.selectedAction
    ...
    this.setState({
      ...
      attachments,
      responsive_tags: [],
      responsive_account_ids: [...responsive_accounts.map(({ id }) => id), ...responsive_tags.map(({ title }) => title)],
    });
  }
    ...
    return (
      ...
      >
        <div onKeyPress={e => {
          ...
          <TitleH1>{t("Edit subaction")}</TitleH1>
          <form id="header" onSubmit={this.submitAction}>
            ...
            <Flex className="btn-group" margin="30px 0 20px">
              ...
              <ButtonPrimary className="" tabIndex="6" margin="0px 0 0 10px"
                type="submit">{t("Update the subaction")}</ButtonPrimary>
            </Flex>
          </form>
        </div>
      </Modal>
