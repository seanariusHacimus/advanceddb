...
import { Button, ButtonPrimary, Flex, InputWrapper, Input, TitleH1 } from '../../../styles';
import { ReactComponent as IconCheck } from '../../../assets/list-icon.svg';
import { fetchActionPlans } from '../../../store/Actions/actions';
import moment from 'moment-timezone';
import { dissoc, indexBy, ErrorAlerts, InputErrors, parseErrors, } from "../../../utils";
import {
  ActionPlanBase,
  errorsConfig,
  getAllTags,
...
class EditActionPlan extends ActionPlanBase {
  ...
  async componentDidMount() {
    const { name, start_at, end_at, responsive_accounts, responsive_tags } = this.props.selectedAction
    const getTags = await getAllTags();
    const allAccounts = await getAllGroupAccounts(this.props.selectedWorkingGroup, responsive_accounts);
    const allTags = indexBy(getTags.map(({ title }) => ({ first_name: title, id: title, isTag: true })), 'id');
    const responsive_account_ids = [...responsive_accounts.map(({ id }) => id), ...responsive_tags.map(({ title }) => title)];
    ...
    if (start_at && end_at && name) {
      try {
        ...
        if (err.message.includes('422')) {
          ...
          const { alerts, errors } = parseErrors(errorsConfig, { ...dissoc(validation, 'action'), ...validation.action });
          this.setState({ alerts, errors })
        }
      }
    } else {
  ...
  render() {
    ...
    const lowerBoundDate = action_boundaries && moment(action_boundaries.start_at)
    const upperBoundDate = action_boundaries && moment(action_boundaries.end_at)
        ...
        <div
          ...
          <form id="header" onSubmit={this.submitAction}>
                  ...
                  <Col span={12}>
                    ...
                    <InputWrapper className='has-messages' align='flex-end'>
                      <DatePicker
                        ...
                        disabledDate={current => lowerBoundDate && current > lowerBoundDate || current > endDate}
                        className={`custom-datepicker large grey ${errors.start_at && 'input-error'} ${start_at ? 'has-value' : ''}`}
                        onChange={val => this.setState({ start_at: val.startOf('day') })}
                      />
                  ...
                  <Col span={12}>
                    ...
                    <InputWrapper className='has-messages' align='flex-end'>
                      <DatePicker
                        ...
                        disabledDate={current => upperBoundDate && current < upperBoundDate || current < startDate}
                        className={`custom-datepicker large grey ${errors.end_at && 'input-error'} ${start_at ? 'has-value' : ''}`}
                        onChange={val => this.setState({ end_at: val.endOf('day') })}
                      />
            ...
            <Flex className="btn-group" margin="30px 0 20px">
              ...
              <ButtonPrimary className="" tabIndex="6" margin="0px 0 0 10px" type="submit">{t("Update the action")}</ButtonPrimary>
            </Flex>
          </form>
        </div>
      </Modal>
