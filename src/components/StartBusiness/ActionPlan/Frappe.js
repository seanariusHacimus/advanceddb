...
class GanttChart extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.data.forEach((item, index) => {
      const { id, name, sub_actions, sub_action_stats, status, start_at, end_at } = item;
      ...
      const parentAction = {
        ...
        progress: (sub_action_stats.completed / sub_action_stats.total * 100) || 0,
  ...
  setCorrectHeightOfTodayHighlight = () => {
    const findTodayLine = this.ganttChartRef?._target?.current?.querySelector('.today-highlight');
  ...
  zoomHandler = type => {
    ...
    if (type === 'in' && zoom < 3) {
      this.setState(prevState => ({ zoom: +parseFloat(prevState.zoom += 0.1).toFixed(1) }));
      ...
      >
        <Flex className="gantt-control">
          <div className="zoom-wrapper">
            ...
            <button onClick={() => this.zoomHandler('out')}><ZoomOutOutlined /></button>
            <button onClick={() => this.zoomHandler('in')}><ZoomInOutlined /> </button>
          </div>
          <select
          ...
          onClick={task => {
            if (task.custom_class.includes('parent')) {
              if (this.state.hiddendActions.includes(task.id)) {
                ...
                const hiddendActions = this.state.hiddendActions.filter(item => item !== task.id);
                this.setState({ hiddendActions }, () => this.sortActions(task.id));
              } else {
                this.setState(prevState => ({ hiddendActions: [...prevState.hiddendActions, task.id] }), () => {
