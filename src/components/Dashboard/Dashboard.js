...
const chartData = (t) => [
  {
    name: 'completed', title: t('Completed tasks'), count: 0, color: '#527BDD',
  },
  {
    name: 'ongoing', title: t('In Progress'), count: 0, color: '#F4D581',
  },
  {
    name: 'ongoingPast', title: t('Past Due'), count: 0, color: '#F4C9D9',
  },
  {
    name: 'notStarted', title: t('Not started'), count: 0, color: '#E5E7EF',
  },
  {
    name: 'onreview', title: t('Under Review'), count: 0, color: '##f3f3f3',
  },
];
...
class Dashboard extends PureComponent {
  ...
  fetchDashboardReport = async () => {
    ...
    try {
      ...
      if (res?.data) {
        const { action_period_stats, action_stats, history: [indicators], indicator_groups } = res.data.data;
        this.setState({
          period: action_period_stats.values,
          actions: action_stats,
          db_score_and_rank: indicators ? indicators?.indicators?.ease_of_doing_business : { rank: constants.doingBusiness.rank, score: constants.doingBusiness.score },
  ...
  modalHandler = () => this.setState((prevState) => ({ showIndicators: !prevState.showIndicators }));

  render() {
    const {
      showIndicators, actions, period, db_score_and_rank, workingGroups
    } = this.state;

    const { t } = this.props
    const { myAccount } = this.props;
    const dbRank = db_score_and_rank[Object.keys(db_score_and_rank).find((key) => key.includes('rank'))].toFixed(1);
    const dbScore = db_score_and_rank[Object.keys(db_score_and_rank).find((key) => key.includes('score'))].toFixed(1);
    ...
    return (
      <DashboardPage ref={this.printRef} id="dashboard">
        <Flex
          ...
          margin="0 0 10px"
        >
          {(myAccount.role === 'superuser' || myAccount.role === 'coordinator') && (
            <ButtonSecondary
        ...
        <Row style={{ marginBottom: 24 }}>
          <Col className="col-left" xs={24} md={8} xl={7} xxl={6}>
            <div className="inner-block col-left">
              <TitleH3>
                {t('Doing Business 2020')}
                {' '}
                {constants.defaultCountry.name}
              </TitleH3>
              <Card
                ...
                actions={[
                  ...
                  >
                    <div className="text-capitalize">{t('See full country profile')} <RightArrow /></div>
                    <div className="text-center"><small>{t("Source: Doing Business")}</small></div>
                  </a>,
                ]}
              >
                ...
                <Divider type="vertical" style={{ borderColor: '#C9CCDD', height: 60, marginRight: 30 }} />
          ...
          <Col className="col-right" xs={24} md={16} xl={17} xxl={18}>
            <div className="inner-block col-right">
              <TitleH3 className="chart-title text-capitalize" margin="auto 0 20px">
                {t('Monthly progress')}
              </TitleH3>
        ...
        <Row>
          <Col className="col-left" xs={24} md={12} >
            <div className="inner-block col-left">
              <TitleH3 margin="auto 0 20px" className="text-capitalize">{t('Overall progress of tasks')} - {moment().format(t('DD MMM YYYY'))}</TitleH3>
              <Row>
          ...
          <Col className="col-right" xs={24} md={12} >
            <div className="inner-block col-right">
              <TitleH3 margin="auto 0 20px">{t('Working Group Progress')}</TitleH3>
              {
                workingGroups.length > 0 ? workingGroups.map((item, index) => {
                  ...
                  if (item.visible) {
                    return (
                      <ProgressBar className="progress" onClick={() => this.props.history.push(`/working-group/${title}`)} key={index}>
