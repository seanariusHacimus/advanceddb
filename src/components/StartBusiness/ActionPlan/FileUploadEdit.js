...
import { ADD_ATTACHMENTS, DELETE_ATTACHMENTS } from '../../../graphql/actions';
import Axios from '../../../utils/axios';
import { ButtonPrimary } from '../../../styles';

const filesToAtachments = data => {
...
function PicturesWall({ setAttachments, id, attachments = [], accept = 'image/png, image/jpeg, .doc, .docx, .pdf', isDisabled = false }) {
  const [t] = useLocale()
  const [error, setError] = useState(null);
  const [data, setData] = useState({
    attachments: [],
    fileList: filesToAtachments(attachments),
  });
  ...
  const addAttachments = async (file) => {
    ...
    let map = {};
    Array(1).fill('').forEach((item, index) => {
    ...
    try {
      ...
      if (data) {
        console.log(data);
        setData(state => ({ ...state, fileList: filesToAtachments(data.data.add_attachments) }));
      }
    }
    catch (err) {
      console.log(err);
  ...
  const removeAttachments = async (attachmentId) => {
    try {
      ...
      if (res?.data) {
        ...
        const fileList = data.fileList.filter(item => item.id !== attachmentId);
