...
import {
  TitleH3, Button, ButtonPrimary, Flex, InputWrapper, Input,
} from '../../../styles';
...
import { dissoc, ErrorAlerts, InputErrors, notEmptyErrorConfig, parseErrors } from "../../../utils";
...
class AddMeetingMinutes extends Component {
  ...
  render() {
    const {
      name, date, newMeetingMinute, attachments, alerts, errors
    } = this.state;
      ...
      <ActionPlanPage>
        <Flex margin="0 0 20px" className="action-btn-group">
          <TitleH3>{t("All meetings")}</TitleH3>
          {
            ...
            && (
              <ButtonPrimary className="small add-new-action" onClick={() => this.setState({ newMeetingMinute: true })}>
        ...
        </Flex>
        {
          ...
          && (
            <form onSubmit={this.submitAction} id="header">
                ...
                <Col xs={24} lg={8}>
                  ...
                  <InputWrapper className='has-messages' align='flex-end'>
                    <DatePicker
                      ...
                      className={`custom-datepicker large dynamic-input grey ${errors.date && 'input-error'} ${date ? 'has-value' : ''}`}
              ...
              <input type="file" multiple onChange={this.fileSelection} hidden ref={(el) => this.fileRef = el} />
              ...
              <Flex className="btn-group">
                <Button className="transparent small" type="button" onClick={() => this.fileRef.click()}>
                  {' '}
                  <img src={iconUpload} alt={t("Upload")} />
                  {' '}
                  {t("Attach file")}
                </Button>
                <Button className="transparent small cancel" type="button"
                  onClick={() => this.setState({ newMeetingMinute: false })}>{t("Cancel")}</Button>
                <ButtonPrimary className="small">{t("Save meeting minutes")}</ButtonPrimary>
              </Flex>
            </form>
          )
        }
