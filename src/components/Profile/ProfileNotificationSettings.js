...
const verboseGatewayNames = (t) => (
  {
    push: {title: t('Advance-DB Portal'), description: 'Receive "%s\" notification in Push and Advance-DB Portal?'},
    email: {title: t('Email'), description: 'Receive "%s\" notification on Email'}
  }
)
...
function NotificationSettings(props) {
  ...
  const onSubmit = async () => {
    try {
      ...
      if (res?.data.data) {
        ...
        Swal.fire({
          title: t('Success'),
          text: t('Your notification settings have been successfully updated'),
          icon: 'success'
        }).then(() => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await Axios.post('/graphql', { query: MY_ACCOUNT_NOTIFICATION_SETTINGS_QUERY });
    ...
    <ProfileEditPage id="profile-page">
      <Flex margin="0 0 30px">
        ...
        <div className="btn-group">
          ...
          <ButtonPrimary className="small" onClick={onSubmit}><img src={iconSave} alt="Profile" /> {t("Save")}</ButtonPrimary>
        </div>
      </Flex>
      <form action="">
        {
          Object.keys(settings).map(notificationType => {
            ...
            const disabledNotifications = ['action_uncompleted', 'action_deleted'];

            if (!disabledNotifications.includes(notificationType)) {
              return (
                <div key={notificationType}>
                  <h4 className="sub-title">{notificationsList(t)[notificationType]}</h4>
                  <Row gutter={[20, 20]}>
                    {Object.keys(setting).map(gateway => {
                      ...
                      return (
                        <Col span={12} key={gateway}>
                          <Flex jc="space-between" className={`switch ${isEnabled && 'checked'}`}>
                            <Tooltip title={t(verbose.description, verboseNotificationType)}>
                              <div className="switch-item-title">{verbose.title}</div>
                            </Tooltip>
                            <Switch size="small"
