...
class Compare extends PureComponent {
      ...
      if (res.data) {
        ...
        for (const x in list) {
          ...
          for (const y in years) {
            const obj = {};
            const value = years[y][0] ? years[y][0]?.indicators[Object.keys(indicatorParent)][x] : [];
            obj.name = y.slice(4);
  ...
  render() {
    ...
    if (data.length) {
      return (
        <CountryPage className="graph-with-box" ref={el => this.customRef = el}>
          <TitleH1 margin="0 0 30px" style={{ display: 'flex', flexWrap: 'wrap' }}>
