...
import { Layout, Menu, Divider, Alert, Badge } from "antd";
...
const { SubMenu } = Menu;
const defaultWorkingGroup = GROUP_NAMES.BUSINESS_ENTRY.id;
...
function HeaderPage(props) {
  ...
  const [t] = useLocale();
  const {
    user: { role, request_password_change, leader_groups, member_groups },
    indicators = [],
      ...
      <Layout style={styles.wrapper}>
        ...
        >
          {links[1] === "working-group" ? (
            ...
            >
              ...
              {!selectedWorkingGroup.removable && [
                <Menu.Item
                  key={`/working-group/${workingGroupUrl}/methodology`}
                >
                  <Link to={`/working-group/${workingGroupUrl}/methodology`}>
                    {t("Methodology")}
                  </Link>
                </Menu.Item>,
                <Menu.Item
                  key={`/working-group/${workingGroupUrl}/historical-data`}
                >
                  <Link
                    to={`/working-group/${workingGroupUrl}/historical-data`}
                  >
                    {t("Historical data")}
                  </Link>
                </Menu.Item>,
                <Menu.Item
                  key={`/working-group/${workingGroupUrl}/what-to-reform`}
                >
                  <Link to={`/working-group/${workingGroupUrl}/what-to-reform`}>
                    {t("What to reform")}
                  </Link>
                </Menu.Item>,
                <Menu.Item
                  key={`/working-group/${workingGroupUrl}/business-comparison`}
                >
                  <Link
                    to={`/working-group/${workingGroupUrl}/business-comparison`}
                  >
                    {t("Compare with other countries")}
                  </Link>
                </Menu.Item>,
              ]}
            </Menu>
            ...
            >
              ...
              <Menu.Item key="/dashboard/db-ranking-simulator">
                <Link to="/dashboard/db-ranking-simulator">
                  {t("DB Ranking Simulator")}
                </Link>
              </Menu.Item>
              <Menu.Item key="/dashboard/reform-update-for-db-team">
                <Link to="/dashboard/reform-update-for-db-team">
                  {t("Reform Update for DB Team")}
                </Link>
              </Menu.Item>
          ...
          <div id="task-bar">
            ...
            <Notifications />
            {/*
               Temporarily hide messaging functionality
