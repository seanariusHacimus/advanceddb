...
import {TitleH1, Button, ButtonPrimary, Flex, InputWrapper, Input} from '../../../styles';
import iconUpload from '../../../assets/indicator/upload.svg';
import Editor from '../../../utils/Editor.js';
import Axios from '../../../utils/axios';
import {UPDATE_MEETING, DELETE_ATTACHMENTS, ADD_ATTACHMENTS} from '../../../graphql/meetings';
...
class ActionPlan extends Component {
  ...
  addAttachments = async () => {
    ...
    let map = {};
    Array(files.length).fill('').forEach((item, index) => {
    ...
    try {
      ...
      if (res?.data) {
        this.setState({
          loading: false,
          attachments: res.data.data.add_attachments.filter(file => file.deleted_at === null),
          msgType: 'success',
  ...
  submitMeetingMinute = async (e) => {
    ...
    const filteredMeetingMinute = Object.keys(meeting_minute).reduce((acc, name) => {
    ...
    try {
      ...
      if (res?.data) {
        ...
        this.props.showMessage('success', t('Meeting minute has been updated successfully'));
        Swal.fire({
          title: t('Updated'),
      ...
      if (err.message.includes('422')) {
        ...
        const {alerts, errors} = parseErrors(errorsConfig,
          {meeting_minute_id: rawErrors.meeting_minute_id, ...rawErrors});
        this.setState({alerts, errors})
      }
    }
  ...
  render() {
    const {name, date, newAttachments, attachments, comment, errors, alerts} = this.state;
      ...
      >
        ...
        <form id="header" onSubmit={this.submitMeetingMinute}>
            ...
            <Col xs={24}>
              <InputWrapper>
                <Input
                  ...
                  className={`dynamic-input grey ${errors.name && 'input-error'} ${name ? 'has-value' : ''}`}
                  onChange={this.handleInput}
                />
                <label htmlFor="" onClick={() => this.nameRef.focus()}>{t("Meeting name *")}</label>
              </InputWrapper>
            </Col>
            <Col span={24}>
              <label htmlFor="" onClick={() => this.end_atRef.click()}>{t("Date *")}</label>
              <InputWrapper>
                <DatePicker
                  required
                  type="date"
                  ...
                  className={`custom-datepicker large grey ${errors.date && 'input-error'}`}
          ...
          <Flex style={{ flexWrap: 'wrap' }}>
            {
              attachments.map(item => {
                return (
                  <div key={item.id} href={item.file.download_url} className="file-attachment">
                    <img src={iconAttachment} alt={t("has attachment")} className="file-attachment-icon" />
                    {item.filename.slice(0, 12) + '...' + item.filename.slice(-5)}
                    <button onClick={(event) => this.removeAttachments(event, item.id)} className="file-attachment-remove-btn">x</button>
                  </div>
                )
              })
            }
          ...
          <Flex className="btn-group" margin="30px 0 20px">
            <Button className="transparent cancel" type="reset" onClick={this.props.modalHandler}>{t("Cancel")}</Button>
            <ButtonPrimary className="" margin="0px 0 0 10px" type="submit">{t("Update Meeting Minutes")}</ButtonPrimary>
          </Flex>
        </form>
      </Modal>
    )
