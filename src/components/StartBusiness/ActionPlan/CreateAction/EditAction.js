...
import { Select, DatePicker, Tag } from "antd"; // Keep for now - complex forms
...
class EditActionPlan extends ActionPlanBase {
  ...
  async componentDidMount() {
    ...
    const topicName = this.props.selectedWorkingGroup?.title;
  ...
  submitAction = async (e) => {
    ...
    const isUserAddedWorkingGroup = Boolean(
      this.props.selectedWorkingGroup?.removable
    );
    ...
    if (hasRequiredFields && hasPillarFields) {
      ...
      if (!Boolean(this.props.selectedWorkingGroup?.removable)) {
      ...
      requiredFields.forEach((item) => {
        ...
        if (
          !this.state[item] ||
          (item === "responsive_account_ids" && this.state[item]?.length === 0)
        ) {
  ...
  render() {
    ...
    const { t } = this.props;
    const topicName = selectedWorkingGroup?.title;
    const isUserAddedWorkingGroup = Boolean(selectedWorkingGroup?.removable);
                ...
                >
                  <ActionNameSelection
                    ...
                    existingActions={this.props.actions
                      .filter(
                        (action) => action.id !== this.props.selectedAction.id
                      )
