...
import {
  FETCH_ALL_MEMBERS,
  DENY_ACCOUNT_MUTATION,
  APPROVE_ACCOUNT_MUTATION,
...
} from "../../assets/startBusiness/add-primary.svg";
import iconApprove, {
  ReactComponent as IconApprove,
} from "../../assets/startBusiness/approve.svg";
import iconDeny, {
  ReactComponent as IconDeny,
} from "../../assets/startBusiness/deny.svg";
            ...
            if (myAccount.role !== "superuser") {
              return !menuItem.admin ? (
                <Menu.Item
                  key={key}
                  onClick={() =>
                    menuItem.action(account, history)?.then(onSuccess)
                  }
                  icon={menuItem.icon}
                >
...
const AccountsList = () => {
  ...
  const [scrollTo, setScrollTo] = useQueryParam(
    "scroll_to_member",
    StringParam
  );
  ...
  const calculatePageOfMember = (memberId) => {
    const actualList = filteredMembers.filter(
      (member) =>
        (filteredStatuses.length === 0 ||
          filteredStatuses.indexOf(member.status) !== -1) &&
        (filteredRoles.length === 0 ||
          filteredRoles.indexOf(member.role) !== -1)
    );
    const indexOfMember = actualList
      .map((member) => member.id)
      .indexOf(memberId);
    return (
      Math.floor((indexOfMember !== -1 ? indexOfMember : 0) / pageSize) + 1
    );
  };
  ...
  const fetchMembers = () => {
    Axios.post("/graphql", {
      ...
      if (res?.data) {
        const newAccounts = res.data.data.accounts.nodes.map((account) =>
          account.type === "dummy"
            ? {
                ...account,
                status: "dummy",
  ...
  const statusColumns = [
    {
      ...
      render: (val, account) => {
        ...
        const leaderOfTheGroup =
          myAccount.role === "participant"
            ? account.member_groups.find((i) =>
        ...
        if (val === "pending" && (hasRights || leaderOfTheGroup))
  ...
  const roleColumn = {
    ...
    render: (val, account) => {
      ...
      const leaderOfTheGroup =
        myAccount.role === "participant"
          ? account.member_groups.find((i) =>
              myAccount.leader_groups.map((groups) => groups.id).includes(i.id)
            )
          : {};
          ...
          expandedRowRender(account) {
            return (
              <div className="expanded-content">
                ...
                {account.leader_groups.concat(account.member_groups).length >
                  0 && (
                  <>
                    <TitleH3 className="expanded-title">
                      {t("Groups")}:
                      {account.leader_groups
                        .concat(account.member_groups)
                        .map((group) => (
                          <a href="#" key={group.id}>
                            {group.title}
                            {", "}
        ...
        {showModal && isUserAllowed(myAccount) && (
          <Suspense fallback={t("Loading...")}>
            <InvitationModal
              ...
              handleSuccess={(email) => {
                toast.success(
                  t(`Your invitation was successfully sent to ${email}`)
                );
                setShowModal(false);
                fetchMembers();
              }}
