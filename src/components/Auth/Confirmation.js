...
export function ConfirmationBase({
  title, subTitle, extras, icon,
}) {
  const [t] = useLocale()
  return (
    <Confirmation>
...
export function ConfirmationExtended() {
  ...
  if (resetId && !reset.status) {
    ...
    title = t('Thank you for verifying your email. You will receive a notification email when your request has been approved.');
    ...
    extra = (
      accountId ? (
        <Button
          ...
          className="btn small"
        >
          <IconReset style={{ verticalAlign: 'middle', marginRight: 7, maxHeight: 50 }} />
          {t("Resend Invite")}
        </Button>
      ) : (
          <Link key="request-access" to="/request-access" className="btn small">
    ...
    subTitle = t('Please check your email to validate your email address. Please check your spam folder to be sure that our emails are not being detected as spam.');
    icon = <IconEmail style={{ minWidth: 50, minHeight: 50 }} />;
    extra = (
      <Button
        key="resend"
