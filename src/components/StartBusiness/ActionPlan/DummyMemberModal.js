...
import { filterVals, roleNames, roleWeights, verboseErrors } from '../../../utils';
...
export default function DummyModal(props) {
  const [t] = useLocale()
  const {
    visible, handleSuccess, handleCancel, input, group_id
  } = props;
            ...
            <Row gutter={[15, 15]}>
              ...
              <Col span={24}>
                <ButtonPrimary className="small"
                  style={{ width: '100%' }}>{t("Create dummy account")}</ButtonPrimary>
              </Col>
            </Row>
          </form>
        )}
