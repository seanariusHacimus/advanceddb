...
export const columns = ({ t, ...props }) => [
    ...
    render: (val, data) => {
      ...
      return (
        <div className="icons-set">
          {data.removable ? data.icon ?
            <img src={data.icon.url || iconUser} className="user-icon" onError={e => e.target.src = iconUser} alt={data.title} />
  ...
  },
  {
    ...
    render: (val, data) => val ?
      ...
      >
        <Tag className="clickable active" style={{ padding: '4px 16px', width: '90%', maxWidth: 150 }}>{t("Active")}</Tag>
      </Popconfirm>
      :
      <Popconfirm
        icon={null}
        ...
        cancelText={t('Cancel')}
      >
        <Tag className="clickable disabled not_started" style={{ padding: '4px 16px', width: '90%', maxWidth: 150, pointerEvents: 'all' }}>{t("Disabled")}</Tag>
      </Popconfirm>,
  },
];
