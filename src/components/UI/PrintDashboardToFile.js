...
import { FETCH_ACTIONS_AND_SUBACTIONS, FETCH_VISIBLE_INIDCATOR_GROUPS } from '../../graphql/print';
import { exportToExel, exportToPdf } from '../../utils/printer';
import { useLocale } from "../../utils/locale";
import { CloudDownloadOutlined, FileExcelOutlined, FilePdfOutlined } from '@ant-design/icons';
...
const PrintToFile = ({ id, title, style, printSortedData = [], page }) => {
  ...
  const groupTitlesForColumn = useMemo(() => groups.map(item => ({ label: item.title, value: item.title })))
  ...
  const allResponsibleTitles = useMemo(() => {
    ...
    data.forEach((item) => {
      ...
      result = result
        .concat(responsive_tags.map(i => ({ label: '#' + i.title, value: i.title })))
        .concat(responsive_accounts.map(i => ({ label: `${i.first_name} ${i.last_name}`, value: i.id })))
    });
  ...
  const filteredAccounts = useMemo(() => {
    console.log(query)
    if (query) {
      return allResponsibleTitles.filter(item => item.label.toLowerCase().includes(query.toLowerCase()))
    } else {
  ...
  const fetchGroups = async () => {
    try {
      ...
      console.log(data.data.indicator_groups);
      setGroups(data.data.indicator_groups.nodes);
    }
    catch (err) {
      console.log(err)
    }
  ...
  const fetchWorkingGroupActions = async (page) => {
    try {
      const { data } = await Axios.post('/graphql', {
        query: FETCH_ACTIONS_AND_SUBACTIONS,
        variables: {
          order: {
            key: "end_at",
            direction: "asc"
      ...
      const result = actions.nodes.map(item => {
        if (item.id in subActionByParentId) {
          return {
            ...item,
            group: item.group.title, sub_actions: subActionByParentId[item.id]
          };
        } else {
          return {
            ...item,
        ...
        return [
          {
            index: index + 1,
            responsive: [
              ...item.responsive_accounts.map(i => ({ label: `${i.first_name} ${i.last_name}`, value: i.id })),
              ...item.responsive_tags.map(i => ({ label: i.title, value: i.title })),
            ],
            ...data
          },
          ...sub_actions.map((i, count) => ({
            ...
            responsive: [
              ...i.responsive_accounts.map(i => ({ label: `${i.first_name} ${i.last_name}`, value: i.id })),
              ...i.responsive_tags.map(i => ({ label: i.title, value: i.title })),
            ],
            ...i,
            group: i.group.title,
          }))
  ...
  const columns = useMemo(() => [
      ...
      filterDropdown: (props) => {
        ...
        return (
          <div className="custom-filter-dropdown">
            ...
            <button style={{ marginTop: 5 }} onClick={handleChange}>{t('Reset')}</button>
          </div>
        )
      },
      onFilter: (value, record) => record.responsive_accounts.find(i => i.id === value) || record.responsive_tags.find(i => i.title === value)
    },
    {
      title: t('Status'),
      dataIndex: 'status',
    ...
    },
    {
      ...
      dataIndex: 'group',
      key: 'group',
      filters: groups.map(item => ({ text: item.title, value: item.title })),
  ...
  return (
    <StyledPrint ref={parentRef} style={{ marginLeft: 10, ...style }}>
      ...
      <Button className="download-actions-button" onClick={handleModal}> <CloudDownloadOutlined /> Download actions</Button>
      <Modal
        visible={visible}
      ...
      >
        <Space size="middle">
          <Button type="primary" onClick={() => exportToPdf({ title, data: printData })}>
            <FilePdfOutlined /> {t("Download PDF")}
          </Button>
          <Button type="primary" onClick={() => exportToExel({ title, data: printData })}>
