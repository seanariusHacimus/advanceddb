...
import CountryPage from '../../styles/compairCountries';
...
class Compare extends PureComponent {
      ...
      if (res.data) {
        ...
        for (const x in list) {
          ...
          for (const y in years) {
            const obj = {};
            const value = years[y][0].indicators[Object.keys(indicatorParent)][x];
            console.log(value);
  ...
  removeCountry = (val) => {
    ...
    const remainingCountries = Object.values(this.state.countries).filter((item) => item.code !== val);
    const data = {};
    remainingCountries.forEach((item) => data[item.code] = item);
    this.setState((prevState) => ({ countries: { ...data } }), () => this.fetchData());
  }
  ...
  render() {
    ...
    if (title === ('Starting a Business')) {
      if (selectedOption) {
        filteredData = data.filter((item) => {
          const notSelectedOption = selectedOption === '_women' ? '_men' : '_women';
          if (item[0].key.toLowerCase().endsWith(selectedOption) && !item[0].key.toLowerCase().endsWith(notSelectedOption)) {
            return item;
          } if (!item[0].key.toLowerCase().endsWith(selectedOption) && !item[0].key.toLowerCase().endsWith(notSelectedOption)) {
            return item;
          }
    ...
    } else if (title === ('Trading Across Borders')) {
      if (selectedOption) {
        filteredData = data.filter((item) => {
          const notSelectedOption = selectedOption === '_export' ? '_import' : '_export';
          if (item[0].key.toLowerCase().includes(selectedOption) && !item[0].key.toLowerCase().includes(notSelectedOption)) {
            return item;
          } if (!item[0].key.toLowerCase().includes(selectedOption) && !item[0].key.toLowerCase().includes(notSelectedOption)) {
            return item;
          }
    ...
    return (
      <CountryPage className="graph-with-box" ref={this.printRef}>
        ...
        <div className="compare-countries sub-title">{t('Select up to 4 countries')}
        </div>
          ...
          </div>
          {
            Object.values(countries).map((item, index) => (
              <div className="country-btn" key={index}>
                <span className="color" style={{ background: colors[index + 1] }} />
                {item.name}
                <button type="button" onClick={() => this.removeCountry(item.code)}>x</button>
              </div>
            ))
          }
          <div id="search">
            <Select
              ...
              onFocus={(e) => e.target.style.cssText = 'width: 100% !important; min-width: 160px;'}
              onSelect={(val) => this.selectHandler(val)}
              // onSearch={onSearch}
              filterOption={(input, option) => option.children.toLowerCase().indexOf(input.toLowerCase()) >= 0 || option.value.toLowerCase().indexOf(input.toLowerCase()) >= 0}
            >
              {
                Object.keys(countriesList).map((item, index) => {
                  if (item !== defaultCountry.code) {
                    return <Option key={index} value={countriesList[item].code}>{countriesList[item].name}</Option>;
                  }
                  return null;
                })
              }
...

        {
          ...
          && (
            <Flex className="filter-btn">
              <Button
                value=""
                className={`btn-group ${selectedOption === '' && 'ant-radio-button-wrapper-checked'}`}
              ...
              <Button
                ...
                onClick={this.handleFilter}
                className={`btn-group ${selectedOption === '_men' && 'ant-radio-button-wrapper-checked'}`}
              >
                {t('Men')}
              </Button>
              <Button
                ...
                onClick={this.handleFilter}
                className={`btn-group ${selectedOption === '_women' && 'ant-radio-button-wrapper-checked'}`}
              >
                {t('Women')}
              </Button>
            </Flex>
          ...
          && (
            <Flex className="filter-btn">
              ...
              <Button
                value="_import"
                onClick={this.handleFilter}
                className={`btn-group ${selectedOption === '_import' && 'ant-radio-button-wrapper-checked'}`}
              >
                {t('Import')}
              </Button>
              <Button
                value="_export"
                onClick={this.handleFilter}
                className={`btn-group ${selectedOption === '_export' && 'ant-radio-button-wrapper-checked'}`}
              >
                {t('Export')}
              </Button>
            </Flex>
