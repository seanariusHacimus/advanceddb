...
import { dissoc, ErrorAlerts, InputErrors, notEmptyErrorConfig, parseErrors, pick } from "../../utils";
...
export class ReformBase extends Component {
  ...
  handleTextArea = val => {
    this.setState({ description: val, errors: dissoc(this.state.errors, 'description') });
  }
}
    ...
    if (filteredReform.group_id) {
      try {
        ...
        if (res?.data.data) {
          ...
          this.props.showMessage('success', t('The reform has been created successfully!'));
          ...
          <form action="">
            ...
            <InputWrapper className='has-messages' align='flex-end'>
              <Select
                ...
                style={{ width: '100%' }}
                optionFilterProp={'children'}
                className={`custom-select narrow no-padding grey ${errors.group_id && 'input-error'} ${group_id ? 'has-value' : ''}`}
                allowClear={true}
                getPopupContainer={node => node.parentNode}
                dropdownStyle={{ backgroundColor: '#535263', padding: 10, }}
              >
