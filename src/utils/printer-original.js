...
function converActionsIntoPrintData(data) {
  ...
  data.forEach((item, index) => {
    ...
    const parentAction = { '#': `${(index + 1)}`, ...capitalizeObject(rest), Responsible: responsible };

    result.push(parentAction);
    sub_actions.forEach((item, subIndex) => {
      const { responsive_accounts, responsive_tags, ...rest } = item;
      const responsible = creatArrayOfStr([...responsive_accounts, ...responsive_tags]);
      const subAction = { '#': `${(index + 1)}.${subIndex + 1}`, ...capitalizeObject(rest), Responsible: responsible };

      result.push(subAction);
    });
  });
  return result;
}
