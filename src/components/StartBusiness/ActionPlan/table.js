        ...
        data.map((item, index) => {
          return (
            <Menu.Item key={item.id}>
              <a href={item.file.download_url} target="_blank">{index + 1}. {item.filename}</a>
            </Menu.Item>
          )
        })
      }
  ...
  return (
    <Dropdown overlay={menu}>
      <div style={{ fontSize: 22 }}>
        <AiOutlinePaperClip size={20} color="#527BDD" style={{ marginRight: -5, marginTop: 5 }} />
      </div>
    </Dropdown>
  )
}

export const columns = ({ completeAction, uncompleteAction, actionPermissions, t }) => [
  {
    title: t('Action name'),
    dataIndex: 'name',
    key: 'name',
          ...
          {action.status === 'completed' ?
            (
              <a type="button" className="trigger-btn">
                <PieChart data={action.sub_action_stats.total ?
  ...
  },
  {
    ...
    render: (val, record) => {
      ...
      const isLeader = myAccount.leader_groups.find(i => i.id === currentWorkingGroupId);
      ...
      const actionApproval = (
        <Popconfirm
          ...
          okText={t("Approve")}
          cancelButtonProps={{ style: { color: '#fff', backgroundColor: 'var(--danger)', borderColor: 'var(--danger)' } }}
          cancelText={t("Reject")}
          icon={null}
        >
          <Button
      ...
      return (record.status === 'on_review' && (myAccount.role !== 'participant' || isLeader)) ?
        actionApproval
        :
        ...
        record.status === 'completed' ?
          ...
          record.status === 'on_review' ?
            ...
            <Popconfirm
              ...
              title={(
                <div>
                  <h3>{t("Submit for Approval?")}</h3>
                  {
                    !isLeader && myAccount.role === 'participant' ?
                      <p>{t("The action status will be under review and needs to be approved or rejected by an authorized person")}</p>
                      :
                      <p>{t("This will also mark related Subactions as Complete.")}</p>
                  }
                </div>
              )}
              onConfirm={(e) => {
...
export const subActionColumn = ({ parentIndex, updateStatus, actionPermissions, t }) => [
  {
    title: t('Action name'),
    dataIndex: 'name',
    key: 'name',
